 ...
 * Customization of the bigplum@gmail.com's original module written by Matteo Zardoni (ma.zardoni@reply.it)
 */
...
static ngx_int_t ngx_http_limit_traffic_rate_filter_handler(ngx_http_request_t *r);
static ngx_int_t ngx_http_limit_traffic_rate_filter_init(ngx_conf_t *cf);
static ngx_int_t ngx_http_limit_traffic_rate_body_filter(ngx_http_request_t *r, ngx_chain_t *in);
static void *ngx_http_limit_traffic_rate_filter_create_loc_conf(ngx_conf_t *cf);
static char *ngx_http_limit_traffic_rate_filter_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child);
static ngx_int_t ngx_http_limit_traffic_rate_filter_init_zone(ngx_shm_zone_t *shm_zone, void *data);
static void ngx_http_limit_traffic_rate_filter_rbtree_insert_value(ngx_rbtree_node_t *temp, ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel);
static void ngx_http_limit_traffic_rate_filter_cleanup(void *data);
static char *ngx_http_limit_traffic_rate_zone(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
static char *ngx_http_limit_traffic_rate(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
ngx_int_t get_bps(char *check_ip);
char *get_user_id(char *args);
ngx_int_t get_bps_user(char *user_id, char *check_ip);
...
static ngx_int_t ngx_http_limit_traffic_rate_filter_handler(ngx_http_request_t *r)
{
    ...
    n = offsetof(ngx_rbtree_node_t, color) + offsetof(ngx_http_limit_traffic_rate_filter_node_t, data) + len;
...
*/
                ...
                char ip[50];         //IP address related to incoming request, in char*
                char args[200];      //args in char*
                size_t i = 0, j = 0; //Index for u_char -> char convertion
                u_char *d;           //IP address related to incoming request, in u_char*, provided by ngx core
                u_char *_args;       //args, in u_char*, provided by ngx core

                //Getting IP address
                d = r->connection->addr_text.data;
                // Getting URI args
                _args = r->args.data;
                while (j < r->args.len)
                {
                ...
                //Convertion from u_char to char (we need char* in order to use strcmp/strcpy)
                while (i < r->connection->addr_text.len)
                {
                    ip[i] = (char)*(d + i);
                    i++;
                }
                ...
                ngx_log_debug8(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                               "limit traffic d:%z n:%O c:%d r:%z:::%z bytes_sent=%z args=%s seconds=%z", lircf->limit_traffic_rate,
                               sent_sum, lir->conn, lir->start_sec, r->limit_rate, r->connection->sent, args, sec);

                if (sec < 60)
                {
                    lircf->limit_traffic_rate = 463000;
                    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "seconds=%z speed:%z", sec, lircf->limit_traffic_rate);
                }
                else
                {
                    //Checking if a user_id is available
                    char *user_id;
                    user_id = get_user_id(args);

                    if (user_id)
                    {
                        // We need to check if a value is available, by consulting $user_id.dat file
                        ngx_int_t ngx_bps = get_bps_user(user_id, ip);
                        if (ngx_bps != -1)
                        {
                            if (lircf->limit_traffic_rate != (size_t)ngx_bps)
                            {
                                lircf->limit_traffic_rate = ngx_bps;
                                r->connection->sent = 0;
                            }
                        }
                        else
                        {
                            lircf->limit_traffic_rate = 50000000;
                        }
                        ngx_log_debug4(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "seconds=%z speed:%z bps=%z user_id=%s", sec, lircf->limit_traffic_rate, ngx_bps, user_id);
                    }
                    else
                    {
                        //Retrieving rate value from config file given the IP address
                        ngx_int_t ngx_bps = get_bps(ip);
                        if (ngx_bps != -1)
                        {
                            if (lircf->limit_traffic_rate != (size_t)ngx_bps)
                            {
                        ...
                        ngx_log_debug3(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "seconds=%z speed:%z bps=%z user_id=NO", sec, lircf->limit_traffic_rate, ngx_bps);
                    }
                }
...
static ngx_int_t ngx_http_limit_traffic_rate_filter_log_handler(ngx_http_request_t *r)
{
    size_t len;
    uint32_t hash;
    ngx_int_t rc;
...
ngx_int_t get_bps(char *check_ip)
{
    ...
    strcpy(filename, "/opt/nginx/conf/addresses/");
    strcat(filename, check_ip);
    strcat(filename, ".dat");
...
char *get_user_id(char *args)
{
    char *query_chunk = NULL;
    char *user_chunk = NULL;
    char *user_id = NULL;

    query_chunk = strtok(args, "&");
    while (query_chunk != NULL)
    {
        if ('u' == *query_chunk)
        {
            user_chunk = query_chunk;
            break;
        }
        query_chunk = strtok(NULL, "&");
    }

    user_id = strtok(user_chunk, "=");
    while (user_chunk != NULL)
    {
        if ('u' != *user_chunk)
        {
            user_id = user_chunk;
...
ngx_int_t get_bps_user(char *user_id, char *check_ip)
{
    ...
    if (file)
    {
        ...
        if (line_chunk != NULL)
        {
            if (strcmp(check_ip, line_chunk) == 0)
            {
                bps = strtok(NULL, ";");
                for (i = 0; i < 20; i++)
                {
